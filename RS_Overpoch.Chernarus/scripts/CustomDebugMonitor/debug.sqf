...
{
	_time = serverTime;     //Want it to count up instead of down? use _time = (round serverTime)/60; //Server countdown _time = (round(120-(serverTime)/60));
    _hours = (floor(_time/60/60));									//_hours = (floor(_time/60));
    _minutes = (floor(_time/60) - (_hours * 60));					//_minutes = (_time - (_hours * 60));
    switch(_minutes) do
    {
        case 9: {_minutes = "09"};
        case 8: {_minutes = "08"};
	...
	<t color='#DD5D25' size='1' font='Zeppelin33' align='left'>Server Uptime: </t><t color='#ffffff' size='1' font='Zeppelin33' align='right'>%9H %10M</t><br/>
	<t color='#ffffff' size='1' font='Zeppelin33' align='center'>discord.gg/xgm8j6z</t>
	",
		(name player),//1
